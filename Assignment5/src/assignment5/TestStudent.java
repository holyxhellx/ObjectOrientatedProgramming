...
public class TestStudent {
    ...
    public static void main(String[] args) throws CloneNotSupportedException, IOException {
        ...
        for (int i = 0 ; i < count ; i++ ) {
            ...
            System.out.println("Number of Students in total: " + Student.getObjectCount());
            if (( TAStudent.getCountTAStudents() + PhDStudent.getCountPhDStudents() ) <= Student.getObjectCount() ) {
                System.out.println("Test of Student count success");
            } else System.out.println("Test of Student count failed");
            System.out.println("");
            System.out.println("");
        }
        //Assignment 5: Console input
        while (true) {
            System.out.println("Input TA, PHD or ST to create new students or");
            System.out.println("Input 'Status' to get the overview printet or");
            System.out.println("Hit return to exit the program");
            BufferedReader inuser = new BufferedReader (new InputStreamReader (System.in));
            String input = inuser.readLine();
            if (input == null || input.length() == 0) return; //Break and end program
            if (input.contains("ST")) { 
                Student student = new Student();
                student.setName("New ST student");
                student.setGroup(0); //not jet in a group (1 or 2)
                student.setProficiencyInJava(0);
                student.setCprNumber(1234000007);
                student.setGender("nn");
                student.printStudent();
            } else if (input.contains("TA")) {
                TAStudent taStudent = new TAStudent();
                taStudent.setName("New TA Student");
                taStudent.setGroup(3); //group 3 i reserve for TA´s
                taStudent.setProficiencyInJava(5);
                taStudent.setCprNumber(1234000007);
                taStudent.setGender("nn");
                taStudent.printStudent();
            } else if (input.contains("PHD")) {
                PhDStudent phDStudent = new PhDStudent();
                phDStudent.setName("New Ph Student");
                phDStudent.setGroup(4); //group 4 i reserve for PhD
                phDStudent.setProficiencyInJava(500);
                phDStudent.setCprNumber(1234000007);
                phDStudent.setGender("nn");
                phDStudent.printStudent();
